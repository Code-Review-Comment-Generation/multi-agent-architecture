...
if __name__ == "__main__":
    ...
    log_path = os.path.join(os.path.dirname(__file__), "logs", repo_name)
    
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    repo_graph = RepoGraph(repo_path=repo_path, save_log=True, log_path=log_path)
    repo_graph.build_whole_graph(repo_path)
    repo_graph.save_graph()
