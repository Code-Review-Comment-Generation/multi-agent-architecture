...
def clone_repo_and_fetch_pr(repo_name, pr_number):
    ...
    clone_dir = repo_name.split('/')[-1] # Use the last part of the repo_name as the directory
    ...
    try:
        ...
        if os.path.exists(clone_dir):
            print(f"Directory ./{clone_dir} might already exist. Attempting to proceed...")
        else:
            return
    ...
    pr_base_ref = f"pull/{pr_number}/merge:pr-{pr_number}-merge-base" # This helps in finding the base
    try:
        ...
        try:
            subprocess.run(["git", "fetch", "origin", f"pull/{pr_number}/merge"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
    ...
    try:
        ...
        for branch_candidate in main_branch_candidates:
            try:
                ...
                subprocess.run(["git", "fetch", "origin", branch_candidate.split('/')[-1]], check=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
                merge_base_process = subprocess.run(
                    ["git", "merge-base", branch_candidate, pr_head_hash],
                    capture_output=True, text=True, check=True
                )
                base_commit = merge_base_process.stdout.strip()
                print(f"Determined base commit for PR #{pr_number} against {branch_candidate}: {base_commit}")
                break
            except subprocess.CalledProcessError:
                continue
    ...
    try:
        ...
        if check_patch_process.returncode != 0:
            print(f"Warning: 'git apply --check' failed. The patch may not apply cleanly.")
            print("Error details:\n", check_patch_process.stderr)
            # Ask user if they want to proceed
            proceed = input("Attempt to apply the patch anyway? (y/N): ").lower()
            if proceed != 'y':
        ...
        print("To create a branch with these changes: git checkout -b pr-{pr_number}-patched")
